...
class Sonido extends abstractModel
{
    ...
    public function getById()
    {
        ...
        $query = $this->Connection->prepare("SELECT * FROM sonido WHERE idsonido = ?");
    ...
    public function create()
    {
        ...
        try {
            $this->getInstance();
            $query = $this->Connection->prepare("INSERT INTO sonido VALUES (null, ?,?,?,?,?)");
            $query->bindParam(1, $this->getCodigo());
            $query->bindParam(2, $this->getDescripcion());
            $query->bindParam(3, $this->getEstado());
    ...
    {
        $this->getInstance();
        $query = $this->Connection->prepare("UPDATE sonido SET codigo = ?,
         descripcion = ?, estado = ?, fecha = ?, activo = ? WHERE idsonido = ?");
        $query->bindParam(1, $this->getCodigo());
        $query->bindParam(2, $this->getDescripcion());
        $query->bindParam(3, $this->getEstado());
        $query->bindParam(4, $this->getFecha());
    ...
    {
        $this->getInstance();
        $query = $this->Connection->prepare("UPDATE sonido SET activo = ? WHERE idsonido = ?");
